...
DISABLE_AWS = os.getenv("DISABLE_AWS", "false").lower() == "true"

if not DISABLE_AWS and not all([AWS_REGION, SQS_QUEUE_URL, DYNAMODB_TABLE_NAME]):
...
try:
    if DISABLE_AWS:
        ...
        log.warning("DISABLE_AWS=true: SQS/DynamoDB desativados; apenas /health estara disponivel.")
    else:
...
def sqs_worker_loop():
    if DISABLE_AWS:
        log.info("Worker SQS desativado por DISABLE_AWS=true.")
        return
